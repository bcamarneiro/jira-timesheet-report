...
describe('generateWeeklyCsv', () => {
	...
	it('should generate CSV with correct headers when no worklogs', () => {
		...
		expect(lines[1]).toBe(
			'Date;Day;Issue Key;Issue Summary;Time Spent (hours);Time Spent (formatted);IsBackdated;DaysLate',
		);
	...
	it('should export a single worklog correctly', () => {
		...
		expect(lines[2]).toBe(
			'2026-03-10;Tue;PROJ-100;Implement feature;2.00;2h;false;0',
		);
	...
	it('should handle CSV special characters in summaries', () => {
		...
		expect(lines[2]).toBe(
			'2026-03-10;Tue;PROJ-100;"Fix ""bug"" with; separator, and commas";1.00;1h;false;0',
		);
	});
	...
	it('should handle missing issue summary', () => {
		...
		expect(lines[2]).toBe('2026-03-10;Tue;PROJ-100;;1.50;1h 30m;false;0');
	});
	...
	it('should use semicolon as delimiter with eight columns', () => {
		const worklogs: WeekWorklogEntry[] = [
			{
				date: '2026-03-10',
				issueKey: 'PROJ-100',
		...
		expect(semicolonCount).toBe(7); // 8 columns = 7 semicolons
	});

	it('should append a week total footer above the provenance line', () => {
		const result = generateWeeklyCsv(
			weekStart,
			weekEnd,
			[
