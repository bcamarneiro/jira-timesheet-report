	...
	for (const issueKey of issues) {
		...
		do {
			...
			if (!resp.ok) {
				...
				throw new Error(
					`Failed to fetch worklogs for ${issueKey}: ${resp.status}`,
				);
			}
			...
			for (const wl of data.worklogs || []) {
				...
				dailyTotals[name][date][issueKey] =
					(dailyTotals[name][date][issueKey] || 0) + wl.timeSpentSeconds;
			}
...
(async () => {
	try {
		...
		console.log(
			`Found ${issues.length} issues with worklogs from ${jqlStartDate} to ${jqlEndDate}`,
		);
		...
		for (const [user, dailyData] of Object.entries(dailyTotals)) {
			...
			let total = 0;
			console.log(`\nUser: ${user}`);
			for (const [date, issueData] of Object.entries(dailyData)) {
				total +=
